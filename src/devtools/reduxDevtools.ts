...
/**
 * Options accepted by {@link reduxDevtools}: Zustand's "devtools" options, except for "name",
 * plus an optional discriminator.
 */
export type ReduxDevtoolsOptions = Omit<DevtoolsOptions, 'name'> & {
  /**
   * Label telling apart several instances of the same store, prepended to the name
   * as "[{discriminator}] MyStore".
   */
 ...
 * e.g. `{ trace: true }`, plus an optional "discriminator" telling apart several instances of
 * the same store: `{ discriminator: 'Left' }` produces "[Left] MyStore". The store "name" is
 * derived from the variable and cannot be overridden here.
 *
